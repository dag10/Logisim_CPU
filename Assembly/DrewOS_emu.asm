																							; .org 000h;
.org 000h; Program originates in RAM location 400h
																							; MANUALLY CHANGE STACK TO 7FFh;
.stack 7FFh; Bottom of stack

; I/O Ports
																							; .map Display, C00h; Terminal display for output
																							; .map Keyboard, C00h; Buffered keyboard for input
.map Display, C00h; Terminal display for output
.map Keyboard, C00h; Buffered keyboard for input
...
.map Rand, C03h; Random number generator
	...
	GetInput:
		JMP (GetInputInit);
			...
			GetInput_lengthTyped:
				.word 0000h; 
			GetInput_length:
				.word 0000h; Length of string to grab
			GetInput_PlainStore:
				.word a000h;
			GetInput_tempChar:
				.word 0000h;
		...
		GetInputLoop:
			LDM (Keyboard); Get input key
			JPZ (GetInputLoop); If key is not down
			
			STM (GetInput_tempChar); If enter is pressed then...
			XOR (KEY_Enter);
			JPZ (GetInputDone);
			...
			DEC (GetInput_length); DEC length, if zero then...
			JPZ (GetInputLoop);
			JPM (GetInputLoop);
...
; PROGRAM START
	...
	InputTest:
		...
		;JMP (TTestNumLoopDone); TEMP
		TTestNumLoop:
			LDM (TTestNum);
			JPZ (TTestNumLoopDone);
			LDI (TTestChar);
			PUSH;
			CALL (Print);
			DEC (TTestNum);
