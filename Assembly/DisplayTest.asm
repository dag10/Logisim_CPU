...
StringEquals:
	JMP (StringEqualsInit);
		StringEqualsRet:
			.word 0000h;
		StringEquals_plainXOR:
			.word 6000h;
		StringEquals_plainLDM:
			.word 9000h;
		StringEquals_HitZero:
			.word 1;
	StringEqualsInit:
		POP; Pop return address
		STM (StringEqualsRet);
		POP; Pop second string
		ADD (StringEquals_plainXOR);
		STM (StringEqualsXORIndex);
		POP; Pop first string
		ADD (StringEquals_plainLDM);
		STM (StringEqualsLDMIndex);
		LDI 1;
		STM (StringEquals_HitZero);
	StringEqualsLoop:
		StringEqualsLDMIndex:
		LDM 0;
		JPZ (StringEqualsHitZero);
		JMP (StringEqualsNotZero);
			StringEqualsHitZero:
			LDI 0;
			STM (StringEquals_HitZero);
		StringEqualsNotZero:
		StringEqualsXORIndex:
		XOR 0;
		JPZ (StringEqualsGoodSoFar);
		JMP (StringEqualsFailed);
			StringEqualsGoodSoFar:
			INC (StringEqualsLDMIndex);
			INC (StringEqualsXORIndex);
			LDM (StringEquals_HitZero);
			JPZ (StringEqualsEquals);
			JMP (StringEqualsLoop);
	StringEqualsEquals:
		LDI 0;
		JPI (StringEqualsRet);
	StringEqualsFailed:
